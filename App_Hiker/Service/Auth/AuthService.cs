...
namespace App_Hiker.Service.Auth
{
    public static class AuthService
    {
        public static async Task<DataResponse<UserDataResponse>> Me()
        {
            ...
            return JsonConvert.DeserializeObject<DataResponse<UserDataResponse>>(response_json)
                ?? new DataResponse<UserDataResponse>();
        }

        public static async Task<LoginResponse> Login(LoginRequest payload)
        {
            string request_json = JsonConvert.SerializeObject(payload);
            string response_json = await ApiService.PostAuth("/auth/login", request_json);
            return JsonConvert.DeserializeObject<LoginResponse>(response_json) ?? new LoginResponse();
        }

        public static async Task<LoginResponse> Refresh(string refreshToken)
        {
            string request_json = JsonConvert.SerializeObject(new RefreshTokenRequest { refresh_token = refreshToken });
            string response_json = await ApiService.PostAuth("/auth/refresh", request_json);
            return JsonConvert.DeserializeObject<LoginResponse>(response_json) ?? new LoginResponse();
        }
        ...
        {
            string request_json = JsonConvert.SerializeObject(new RefreshTokenRequest { refresh_token = refreshToken });
            await ApiService.PostAuth("/auth/logout", request_json);
        }
        ...
        public static async Task<MessageResponse> ForgotPassword(string email)
        {
            ...
            string response_json = await ApiService.PostAuth("/auth/forgot-password", request_json);
        ...
        {
            string request_json = JsonConvert.SerializeObject(payload);
            string response_json = await ApiService.PostAuth("/auth/reset-password", request_json);
            return JsonConvert.DeserializeObject<MessageResponse>(response_json) ?? new MessageResponse();
        }
    }
}
